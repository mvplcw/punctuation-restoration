...
def get_text_decodings(texts, parameters):
  ...
  while True:
    for j in range(0, len(x), batch_size):
      ...
      for i, (input_text, target_text) in enumerate(zip(x[j:j+batch_size], y[j:j+batch_size])):
        ...
        encoder_input_data[i] = get_text_encodings([input_text],parameters)
        decoder_input_data[i] = get_text_decodings([target_text],parameters)
        decoder_target_data[i] = get_text_decodings([target_text[1:]],parameters)
...
def decode(model, parameters, input_texts, allowed_extras, batch_size):
  ...
  while i < parameters["max_decoder_length"]:
    ...
    #continue or finish
    if sampled_chars == '\n' or i == parameters["max_decoder_length"] - 1 or curr_char_index is len(input_texts):
      break
...
def training():
  ...
  for i in train_corpus:
    target_text = '\t '+i +' \n'  
    train_corpus_output.append(target_text)
  ...
  for sentence in train_corpus_output:
    ...
    #find all characters for input and output
    for char in newtext:
      if char not in input_character:
        input_character.append(char)
    for char in sentence:
      if char not in output_character:
        output_character.append(char)

  #sort the character in ascending order
  input_character = sorted(input_character)
  output_character = sorted(list(output_character))

  #Calculate the longest length for input and output sentence 
  input_longest_sentence = len(max(train_corpus_input, key=len))
  output_longest_sentence = len(max(train_corpus_output, key=len))

  #Tokenization for input and output
  encoder_token = tf.keras.preprocessing.text.Tokenizer(filters='', lower=False, num_words=None, char_level=True)
  encoder_token.fit_on_texts(train_corpus_input)
  decoder_token = tf.keras.preprocessing.text.Tokenizer(filters='', lower=False, num_words=None, char_level=True)
  decoder_token.fit_on_texts(train_corpus_output)

  #get reverse the word and its index
  reverse_encoder_dict = {i: c for c, i in encoder_token.word_index.items()}
  reverse_decoder_dict = {i: c for c, i in decoder_token.word_index.items()}

  #set the values of parameters
  parameters = {'max_encoder_length':input_longest_sentence, 'max_decoder_length':output_longest_sentence, 'encoder_size':len(input_character)+1 ,'decoder_size':len(output_character)+1 , 'reverse_encoder_token':reverse_encoder_dict , 'reverse_decoder_token':reverse_decoder_dict , 'encoder_token':encoder_token , 'decoder_token':decoder_token  }

  #call the save_params function to save the parameters
  save_params('character_parameters.pkl',parameters)
  ...
  #save the model after the training have completed
  save_model(model,'character_model.h5')

#function to accept input for prediction and output the result
def testing():
  #load model and parameters for testing
  model,param = load_for_test('character_model.h5','character_parameters.pkl') # change to your path
